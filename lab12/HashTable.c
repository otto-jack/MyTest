...
void HashTableStats(HashTable ht)
{
	...
	int maxLength = 0;
	for (i = 0; i < ht->nslots; ++i) {
		if (maxLength < ListLength(ht->lists[i])) maxLength = ListLength(ht->lists[i]);
	}
	int *lengthDistribution = calloc(maxLength + 1, sizeof(int));
	for (i = 0; i < ht->nslots; ++i) {
		lengthDistribution[ListLength(ht->lists[i])]++;
	}
	for (i = 0; i <= maxLength; ++i) {
		printf("%8d %8d\n",i ,lengthDistribution[i]);
	}
}
